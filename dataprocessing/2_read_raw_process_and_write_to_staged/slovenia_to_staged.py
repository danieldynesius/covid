...
df['channel'] = df['statistična_regija'].str.lower()
df['datum'] = pd.to_datetime(df.datum)

df["week"]   = (df['datum'].dt.isocalendar().week).astype(str)
df["week_no"]= (df['datum'].dt.isocalendar().week).astype(str)

df['datum'] = df['datum'].astype(str)

df["year"] = df["datum"].str[:4].astype(int)
...
metric_nm = "conc_sars_n3_raw"
df.rename(columns={"conc_sars_n3_raw": "value", }, inplace=True) # relative_copy_number
